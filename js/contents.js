document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelector('.lecture-list').addEventListener('click', function(event) {
    ...
    .then(data => {
        if (data.status === 'OK') {
            data.data.forEach(content => {
                ...
                displayViewer({ name: content.fileName, type: content.fileType, url: content.fileUrl }, viewerDiv);
            });
        }else {
            alert('콘텐츠 불러오기 실패');
        }
