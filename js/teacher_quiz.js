document.addEventListener("DOMContentLoaded", function() {
    ...
    async function fetchPdfContents() {
        try {
            const response = await fetch('http://localhost:8080/teacher/course/contents/list', {
                method: 'GET',
                headers: {
    ...
    function displayQuizResults(data) {
        ...
        saveButton.addEventListener("click", async function () {
            const selectedQuizzes = quizListContainer.querySelectorAll(".quiz-select:checked");
            ...
            const quizzesToSave = Array.from(selectedQuizzes).map((checkbox) => {
                ...
                const options = Array.from(quizDiv.querySelectorAll(".quiz-opt input")).map((input) => input.value.trim());
            ...
            try {
                const response = await fetch('http://localhost:8080/teacher/quiz/generate', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
